...
async function checkUserBonus(incomingName) {
  ...
  function checkEqualsDay(dateOne, dateTwo) {
    ...
    const existingBonus = areDatesEqual
      ? "userExist_bonusExist"
      : "userExist_bonusNotExist";
  ...
  try {
    ...
    const allUsersWithBonus = await bonuses.find({
        name: incomingName,
    }).sort({date:1}).toArray();
    console.log(allUsersWithBonus);
    // console.log("--------------");
    // console.log("incomingName - ", incomingName);
    // console.log("foundName - ", oneUser);
    // console.log("--------------");
